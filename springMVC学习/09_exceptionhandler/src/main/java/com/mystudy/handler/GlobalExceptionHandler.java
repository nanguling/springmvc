 ...
 *      位置：在类的上面
 *      特点：必须让框架知道这个注解所在的包名，需要在springmvc配置文件声明组件扫描器(指定@ControllerAdvice所在包名)。
 */
@ControllerAdvice
public class GlobalExceptionHandler {
    ...
    //定义方法处理异常
    /*
        处理异常的方法和控制器方法的定义一样，可以有多个参数，可以有ModelAndView，String，void和Object的返回值。

        形参：Exception e，表示Controller中抛出的异常对象。
             通过形参可以获取发生的异常信息。

        @ExceptionHandler(异常的class)：表示异常的类型，当发生此类型异常时，由当前方法来处理。
     */
    @ExceptionHandler(value = NameException.class)
    public ModelAndView doNaemException(Exception e) {
        //处理NameException异常
        /*
            异常发生处理逻辑：
                1.需要记录异常，记录到数据库，日志文件。
                  记录异常发生的时间，哪个方法发生的，异常的错误内容。
                2.发送通知，把异常的信息通过邮件，短信或者微信发送给相关人员。
                3.给用户友好的提示。
         */
        ModelAndView mv = new ModelAndView();
        mv.addObject("message","用户名不为zs！");
